...
func usage() {
	...
	fmt.Fprintf(os.Stderr, "       %s build [options]\n\n", os.Args[0])
	fmt.Fprintln(os.Stderr, "Extract mode:")
	flag.PrintDefaults()
	fmt.Fprintln(os.Stderr, "\nBuild mode:")
	fmt.Fprintln(os.Stderr, "  -partitions string")
	fmt.Fprintln(os.Stderr, "        Comma-separated list of partition:image pairs (required)")
	fmt.Fprintln(os.Stderr, "  -output string")
	fmt.Fprintln(os.Stderr, "        Output payload file (default \"payload.bin\")")
	os.Exit(2)
}
